...
def str_tags_to_binary_tensor(los: list[str]) -> list[int]:
    """Convert list of strings to indexed positions. """
    arr = np.zeros(len(LABEL_TYPES))
    for str_ in los:
        if not str_ in LABEL_TYPES: continue
        
        # It's in our list. Get the label. Mark as 1 in our label list. 
        arr[LABEL_TYPES.index(str_)] = 1

    return arr
    
def get_span_indicies(offset_mapping: list[(int, int)], st_ch_index: int, end_ch_index: int) -> (int, int):
...
def load_data(clip_path: str) -> pd.DataFrame:
...
def split_into_chunks(tokenizer, df: pd.DataFrame, replace_text_with_tags: bool, max_seq_length: int) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        for sentence in row.labels:
            ...
            # Tokenize the text
            label_arr = str_tags_to_binary_tensor(sentence[2].split(', '))
            model_inputs = tokenizer(text, max_length=max_seq_length, truncation=True)
        
            # Add to our lists
            model_inputs['id'] = row.note_id
            model_inputs['labels'] = label_arr 
...
def get_data(tokenizer, clip_path: str, replace_text_with_tags: bool, max_seq_length: int) -> DatasetDict:
    """Get the CLIP data. 
...
def compute_metrics(predictions):
    ...
    preds = np.asarray(predictions.predictions >= 0.5, dtype=int)
...
if __name__ == '__main__':
    ...
    config = AutoConfig.from_pretrained(args.model_path, num_labels=len(LABEL_TYPES))
